...
- [ ] **Build Verification**: `npm run build` compiles clean with zero TypeScript errors or warnings.
- [ ] **Visual Parity**: 5-tile hero gallery grid, sticky navigation bar, booking card layout, and typography accurately match the reference.
- [ ] **Asset Delivery**: All 43 photos load reliably in WebP format with zero broken images.
- [ ] **State Machine Integrity**: Seamless transitions between Listing Page, Photo Tour, and Lightbox without state de-sync.
- [ ] **Keyboard Navigation**: `ArrowLeft`, `ArrowRight`, `Escape`, `Tab`, and `Enter` functional on all interactive modules and dialogs.
- [ ] **Focus Management**: Focus strictly contained within modals and restored to trigger on close.
- [ ] **Storage Persistence**: Wishlist state persists across browser page reloads via `localStorage`.
- [ ] **Edge Cases**: Zero guests / maximum guests limits enforced; date range inversion prevented; first/last image boundaries handled.
