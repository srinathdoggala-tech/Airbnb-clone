...
| **Hero Photo Gallery** | 5-tile asymmetric layout: 1 large photo (2 rows, left) + 4 smaller square photos (right grid 2x2). Rounded outer corners, "Show all 43 photos" bottom-right button. | `HeroGallery.module.css` with CSS Grid (`grid-template-columns: 2fr 1fr 1fr; grid-template-rows: repeat(2, 210px); gap: 8px; border-radius: 12px; computed height 428px`). Hover dimming and scale. | **MATCH** |
